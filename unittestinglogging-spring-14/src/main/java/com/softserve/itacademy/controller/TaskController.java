...
public class TaskController {
    ...
    public String create(@PathVariable("todo_id") long todoId, Model model) {
        logger.info("Get method, redirect to page 'Create task'");
    ...
    public String create(@PathVariable("todo_id") long todoId, Model model,
        ...
        logger.info("Post method, Create task by ToDo id");
        if (result.hasErrors()) {
            model.addAttribute("todo", todoService.readById(todoId));
            model.addAttribute("priorities", Priority.values());
            return "create-task";
    ...
    public String update(@PathVariable("task_id") long taskId, @PathVariable("todo_id") long todoId, Model model) {
        logger.info("Get method, redirect to page 'Update task'");
    ...
    public String update(@PathVariable("task_id") long taskId, @PathVariable("todo_id") long todoId, Model model,
                         @Validated @ModelAttribute("task")TaskDto taskDto, BindingResult result) {
        logger.info("Post method, Update task by ToDo id");
        if (result.hasErrors()) {
            model.addAttribute("priorities", Priority.values());
            model.addAttribute("states", stateService.getAll());
            return "update-task";
    ...
    public String delete(@PathVariable("task_id") long taskId, @PathVariable("todo_id") long todoId) {
        logger.info("Get method, Delete task by ToDo id");
